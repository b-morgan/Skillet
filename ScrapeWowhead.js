...
// [Paste] into console for each profession page
...
function get_skill_levels() {
    $('div.listview > div:visible .listview-row').each(function(i, row){
        ...
        // Almost all skills in wowhead only have 3 numbers shown
        // Dunno if this is a bug or not, but pad them to 4
        if (arr.length < 4) {
...
function main_loop() {
    ...
    get_skill_levels();
    // DEBUGGING:
    //console.log('current-tab: ' + $tabs.eq(current_tab).text() + ' >>> ' + $nav.find('span').text());
    ...
    if ($next_page.attr('data-active') == 'yes') {
        ...
        $next_page.click();
        setTimeout(main_loop, 250); // Wait a moment to give it time to load
        return;
